...
const fetchLikedDiaries = async (userId) => {
  try {
    const likedDiaries = await (
      await axios.get(`https://localhost:7030/api/DiaryLikes/user/${userId}`)
    ).data;
    return likedDiaries;
  } catch (error) {
    console.log("Error fetching liked diaries:", error);
    return [];
  }
};
        ...
        <div className="profileMain">
          <div className="sideBar">
            ...
            <ul>
              ...
              <li className="menuItem">
                <Link to="/home">Delete account</Link>
              </li>
            </ul>
          </div>
          <div className="diaries">
            {likedDiaries &&
              ...
              likedDiaries.map((item, index) => (
                <PostInFeed key={index} data={item} />
              ))}
          </div>
        </div>
      </div>
